...
@app.route('/')
def greeting():
   return render_template('greeting.htm')
...
def addrec():
   if request.method == 'POST':
      try:
         ...
         with sql.connect(host="localhost", user="flask", password="password", database="client_db") as con:
            cur = con.cursor()
            cmd = "INSERT INTO clients (ClientID,ClientName,ClientPhone,ClientWatch,TotalSpent) VALUES ('{0}','{1}','{2}','{3}','{4}')".format(ClientID,ClientName,ClientPhone,ClientWatch,TotalSpent)
            cur.execute(cmd)
